...
// 자바스크립트는 숫자를 0부터 처리한다. 그래서 getMonth로 월 출력 시 +1을 반드시 작성해야 한다.
let now = new Date(); //함수
...
console.log(`yoil 배열 데이터 개수는 ${yoil.length}개이다.`);
//배열 생성과 호출 연습
let colorSet = ['blue','purple','yellow','red','orange'];
console.log(colorSet[0],colorSet[2],colorSet[4]); //랜덤하게 3개 고르기
...
//coffeeMenu배열의 0인덱스 데이터값을 cart배열의 마지막 위치에 push추가한다.
console.log(cart);
//카페라떼 1잔 구입
...
//최종 결제 금액은 ?원입니다.
console.log(`최종 결제 금액은 ${coffeePrice*cart.length}원입니다.`);
console.log('=============================');
//배열 작성 문법 연습
