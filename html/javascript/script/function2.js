...
//3-1. (함수 안) 함수 실행 시마다 위 1번의 입력값을 받기 위해 '매개변수' 생성
...
console.log(typeof danInput.value);
function dan99(dan){
    let total = '';
...
console.log(nav_a2[1].parentElement); //DOM 요소 nav
...
//style을 같은 DOM요소에 연속해서 두번 이상 적용하고 싶은 경우 CSS속성명을 자바스크립트 키워드 방식으로 Camel표기법으로 작성해야 한다.
nav_a1.parentElement.style.backgroundColor = 'aqua';
nav_a1.parentNode.style.padding = '20px';
nav_a1.addEventListener('mouseover',()=>nav_a1.parentElement.style.borderBottom = 
'2px solid #000');
nav_a2[3].addEventListener('mouseover',()=>nav_a2[3].parentNode.style.backgroundColor='beige');
...
//header에 마우스 올렸을 때 자식 h1의 logo -> '로고' 글자변경하기
header.addEventListener('mouseover',()=>{
    //console.log(header.children[0].children[0].innerText)
    header.children[0].children[0].innerText='<em>로고</em>'
})
console.log('============childElementCount 자식노드개수')
console.log(header.childElementCount); //2

//===========================마켓컬리 쇼핑몰 장바구니 개수
//1. "cartNum" 장바구니 초기값 0
//2. "cartItem" 사용자가 장바구니에 물건 2개를 담았다.
//3. "cartNumAll" 2번의 물건 개수에 따라 장바구니 페이지에 전체 상품 개수 출력
let cartNum = 0;
const cartNumAll = document.querySelector('.all_cart_num');
const cartItem = document.querySelector('.cart_contents');
cartNumAll.innerText = cartNum; //초기값 0(상황 1)
//사용자가 물건 2개를 담았다(상황 2)
cartNum = cartItem.childElementCount;
console.log(cartNum);
cartNumAll.innerText = cartNum; //현재 진행기준 장바구니 개수 대입
